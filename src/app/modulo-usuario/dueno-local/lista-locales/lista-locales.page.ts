...
import { ConfDeleteLocalDialogComponent } from '../../../modulo-admin/Locales/components/conf-delete-local-dialog/conf-delete-local-dialog.component'
...
export class ListaLocalPage implements OnInit {
  ...
  getLocales(){
    this.api.get("Locales").subscribe((locales)=>{
      ...
      this.lstLocales =  this.lstLocales.filter((local:any)=> local.usuario == this.datosUsuario.id)
    });
  }
  ...
  detalle(item:any){
    const navigationExtras: NavigationExtras = {
      state: {
        item: item,
        paginaAnterior:'/tabs/dueno-local/lista-locales'
      }
    };
    this.eventos.recargarLocal.emit(true);
    this.router.navigate(['/tabs/dueno-local/modificar-local'], navigationExtras);
  }
  ...
  detalle2(item:any){
    ...
    this.router.navigate(['/tabs/dueno-local/lista-productos'], navigationExtras);
  }
  ...
  goAgregarLocal() {
    this.router.navigate(['/tabs/dueno-local/agregar-local']);
